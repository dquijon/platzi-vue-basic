		...
		<template v-if="!isLoading">
			<div class="flex flex-col sm:flex-row justify-around items-center">
				...
				>
					...
					<span class="text-xl"
						>{{ convertResult }}
						{{ this.fromUsd ? asset.symbol : "USD" }}</span
					>
				</div>
			</div>
			...
			<table>
				<tr
					v-for="m in markets"
					:key="`${m.exchangeId}-${m.priceUsd}-${m.baseId}`"
					class="border-b"
				>
					<td>
						<b>{{ m.exchangeId }}</b>
...
export default {
	...
	computed: {
		...
		convertResult() {
			...
			return this.fromUsd
				? (this.convertValue / this.asset.priceUsd).toFixed(4)
				: (this.convertValue * this.asset.priceUsd).toFixed(4);
		}
	},
	...
	methods: {
		...
		getWebSite(market) {
			...
			return api
				...
				.then(exchange => {
					this.$set(market, "url", exchange.exchangeUrl);
				})
				.finally(() => this.$set(market, "isLoading", false));
		},
